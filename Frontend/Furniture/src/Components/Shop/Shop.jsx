        ...
        cart.map(item => (
          <div key={item.productId._id} className="cart-item grid grid-cols-4 cartItems">
            ...
            <div className='grid grid-cols-7 ' >
                <button className='borderCircle' onClick={() => updateQuantity(item.productId._id, item.quantity - 1)} disabled={item.quantity <= 1}>−</button>
                <span>{item.quantity}</span>
                <button className='borderCircle' onClick={() => updateQuantity(item.productId._id, item.quantity + 1)}>+</button>
            </div>
            <div>
              <button onClick={() => removeItem(item.productId._id)} className="deleteBtn">Delete</button>
            </div>
          </div>
